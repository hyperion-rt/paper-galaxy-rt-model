...
t_arm = atpy.Table('tables/updated_arms.tbl')
...
for ispec in range(n_spec):
    ...
    for iarm in range(n_arm):
        ...
        if t_arm['Arm'][iarm].strip() in ["2'", "2"]:
            if t['Type'][ispec][0:2] in ['B0', 'B2', 'B5', 'YO']:
                dfactor = 2.
            else:
                dfactor = 1.
        elif t_arm['Arm'][iarm].strip() in ["1'", "1"]:
            if t['Type'][ispec][0:2] in ['B0', 'B2', 'B5', 'YO']:
                dfactor = 1.
            else:
                dfactor = 0.
        ...
        if t_arm['Arm'][iarm].strip() in ["1'", "1", "2'", "2"]:

            in_arm = (R < R_arm_max) & (R > t_arm['R_min'][iarm])

            rho[in_arm] += rho_a[ispec] * dfactor \
                         * np.exp(-(R[in_arm] - R_arm[in_arm]) ** 2 \
                                  / (2. * (0.55) ** 2)) \
...
hdulist.writeto('models/sky_model_updarm_gaussian.fits', clobber=True)
